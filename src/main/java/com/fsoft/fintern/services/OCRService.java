    ...
    public void saveToDatabase(Map<String, List<String>> cvData, int fileId, int recruitmentId) {
        ...
        if (!cvData.get("GPA").isEmpty()) {
            ...
            try {
                ...
                Float gpaValue = Float.parseFloat(gpaParts[0]); // Lấy giá trị GPA (8.0 hoặc 3.2)
                Float gpaScale = Float.parseFloat(gpaParts[1]); // Lấy hệ (10 hoặc 4)

                if (gpaScale == 10.0f) {
                    // Chuyển từ hệ 10 sang hệ 4
                    gpaValue = (gpaValue / 10.0f) * 4.0f;
                }
                // Nếu là hệ 4, giữ nguyên
                cvInfo.setGpa(gpaValue);
            } catch (NumberFormatException | ArrayIndexOutOfBoundsException e) {
                cvInfo.setGpa(0.0f); // Giá trị mặc định nếu không parse được
            }
        } else {
            cvInfo.setGpa(0.0f); // Giá trị mặc định nếu không có GPA
        }
